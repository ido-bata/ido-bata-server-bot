...
- `STATE_SNAPSHOT_UPLOAD_REPO` (or `GITHUB_REPOSITORY`), `STATE_SNAPSHOT_UPLOAD_BRANCH` (default `state-snapshots`), and `STATE_SNAPSHOT_UPLOAD_TOKEN` (or `GITHUB_TOKEN`) configure the GitHub destination. The repo and branch are not secrets, but reviewers should confirm the destination branch is dedicated to snapshots and not used for any other content. The upload token is a `contents:write` PAT or installation token; treat it as a secret and never commit it.

## Snapshot upload path

The bot uses the GitHub REST Git Data API (`/repos/{owner}/{repo}/git/refs`, `/git/commits`, `/git/blobs`, `/git/trees`) to publish each encrypted snapshot to a dedicated branch. This avoids shelling out to a `git` binary or mutating a live worktree, so the uploader works in container runtimes that do not ship `git` or a `.git` directory.
