...
This file is the dispatcher for AI coding agents (Codex, Claude Code, or any local subagent supervisor) when working on `ido-bata-server-bot`. Load other docs from `docs/` only when you need them — do not re-load this meta-prompt or the full project-init document.
