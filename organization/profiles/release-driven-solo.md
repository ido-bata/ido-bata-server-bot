...
ido-bata-server-bot の current release-driven development で使用する Operating Model を定義します。
...
- Bun / Node runtime、Discord integration、既存 `docs/process.md` / `docs/quality.md` / `docs/security.md` を project-specific Practice として保持する。
- Discord credential / guild state を repository や agent result に永続化せず、secret provider と runtime injection の境界を維持する。
- operator runtime gate を release evidence として source-only CI と分離する。
